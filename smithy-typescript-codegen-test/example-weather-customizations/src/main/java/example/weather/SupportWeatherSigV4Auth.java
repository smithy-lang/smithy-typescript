...
public final class SupportWeatherSigV4Auth implements HttpAuthTypeScriptIntegration {
  static final Symbol AWS_CREDENTIAL_IDENTITY =
      Symbol.builder()
          ...
          .build();
  static final Symbol AWS_CREDENTIAL_IDENTITY_PROVIDER =
      Symbol.builder()
          .name("AwsCredentialIdentityProvider")
          .namespace(TypeScriptDependency.SMITHY_TYPES.getPackageName(), "/")
          .addDependency(TypeScriptDependency.SMITHY_TYPES)
          .build();
  static final ConfigField CREDENTIALS_CONFIG_FIELD =
      ConfigField.builder()
          .name("credentials")
                ...
                ConfigField.builder()
                    ...
                    .inputType(
                        Symbol.builder()
                            ...
                            .build())
                    .resolvedType(
                        Symbol.builder().name("__Provider<string>").addReference(PROVIDER).build())
                ...
                HttpAuthSchemeParameter.builder()
                    ...
                    .source(
                        w -> {
                          ...
                          w.addImport(
                              "normalizeProvider", null, TypeScriptDependency.UTIL_MIDDLEWARE);
                          w.openBlock(
                              "await normalizeProvider(config.region)() || (() => {",
                              "})()",
                              () -> {
                                w.write(
                                    "throw new Error(\"expected `region` to be configured for"
                                        + " `aws.auth#sigv4`\");");
                              });
                        })
                    .build())
                ...
                HttpAuthOptionProperty.builder()
                    ...
                    .source(
                        s ->
                            w -> {
                              w.write(
                                  "$S", s.trait().toNode().expectObjectNode().getMember("name"));
                            })
                    .build())
            .addHttpAuthOptionProperty(
                HttpAuthOptionProperty.builder()
