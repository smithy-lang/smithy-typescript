...
import { createNestingWidget, nestingWidget, unionStruct, unionStructControl, widget } from "../test-schema.fixture";
...
describe(JsonShapeDeserializer.name, () => {
  ...
  it("deserializes big integers from string or number", async () => {
    expect(
      ...
      bigint: BigInt("1000000000000000000000000000000000000"),
    });

    const impreciseParsing = BigInt("1000000000000000042420637374017961984");
    ...
    expect(
      ...
      bigint: contextSourceAvailable ? BigInt("1000000000000000000000000000000000000") : impreciseParsing,
    });
  });
