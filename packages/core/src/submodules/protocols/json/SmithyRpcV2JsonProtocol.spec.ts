...
import { SmithyRpcV2JsonProtocol } from "./SmithyRpcV2JsonProtocol";

describe(SmithyRpcV2JsonProtocol.name, () => {
  const bytes = (arr: number[]) => Buffer.from(arr);
  const b64 = (arr: number[]) => toBase64(new Uint8Array(arr));
  const jsonBody = (value: any) => new TextEncoder().encode(JSON.stringify(value));
  ...
  describe("serialization", () => {
    ...
    for (const testCase of testCases) {
      it(`should serialize HTTP Requests: ${testCase.name}`, async () => {
        const protocol = new SmithyRpcV2JsonProtocol({ defaultNamespace: "" });
        ...
        expect(JSON.parse(body)).toEqual(testCase.expected.body);
      });
    }
  });
  ...
  describe("deserialization", () => {
    ...
    for (const testCase of testCases) {
      it(`should deserialize HTTP Responses: ${testCase.name}`, async () => {
        const protocol = new SmithyRpcV2JsonProtocol({
          defaultNamespace: "",
        });
        const output = await protocol.deserializeResponse(
          ...
          new HttpResponse({
            statusCode: 200,
            body: jsonBody(testCase.mockOutput),
          })
        );
  ...
  describe("error handling", () => {
    const protocol = new SmithyRpcV2JsonProtocol({ defaultNamespace: "ns" });
