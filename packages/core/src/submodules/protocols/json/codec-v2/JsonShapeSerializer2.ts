...
export class JsonShapeSerializer2 extends SerdeContextConfig implements ShapeSerializer<Uint8Array> {
  ...
  protected writeValue(schema: Schema, value: unknown, container: NormalizedSchema | undefined): void {
    ...
    // When bigNumberAsString is enabled (Smithy RPCv2 JSON), bigInteger and
    // bigDecimal values are serialized as JSON strings to preserve arbitrary
    // precision, regardless of the incoming JS representation (number, bigint,
    // NumericValue, big.js, or string). Otherwise (AWS JSON 1.0/1.1) they fall
    // through to the default unquoted-number handling below.
    if (this.settings.bigNumberAsString && value != null && (ns.isBigIntegerSchema() || ns.isBigDecimalSchema())) {
