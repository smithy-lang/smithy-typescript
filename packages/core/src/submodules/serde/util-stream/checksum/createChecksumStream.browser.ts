...
export const createChecksumStream = ({
  ...
  const transform = new TransformStream({
    ...
    async flush(controller: TransformStreamDefaultController) {
      ...
      let received: string;
      try {
        if (typeof expectedChecksum === "function") {
  ...
  const observed = new ReadableStream(
    {
      async pull(controller) {
        ...
        try {
          ...
          /**
           * A read failure of any kind, whether malformed framing or a
           * transport interruption, means the comparison never ran over
           * complete bytes. Validation fails rather than being reported as an
           * incomplete read the consumer chose.
           */
          heldChunk = undefined;
          settle({
            status: "FAILED",
