...
export interface ChecksumStreamInitBase {
   ...
   * been selected.
   *
   * Every source failure reported before the comparison runs settles as
   * `FAILED`, whether it is malformed framing, a transport error, or a
   * connection lost before the end of the body. `INCOMPLETE` is reserved for
   * teardown the consumer initiated.
   *
