 ...
 * - `INCOMPLETE`: the consumer cancelled, destroyed, or closed the body before
 *   its normal end of stream, so no comparison ran. This covers only teardown
 *   the consumer initiated; a failure of the body itself is `FAILED`.
 * - `FAILED`: the comparison ran and the checksums differed, or it could not
 *   run because the response framing could not be decoded, the body failed
 *   before its end of stream, the expected value could not be obtained, the
 *   digest could not be calculated, or the digest could not be Base64-encoded.
 *
 * @public
 */
...
export interface ChecksumValidationResult {
   ...
   * was eligible, when the body ended or failed early, when response framing
   * could not be decoded, when the expected value could not be obtained, when
   * the digest could not be calculated, or when the digest could not be
   * Base64-encoded.
   */
