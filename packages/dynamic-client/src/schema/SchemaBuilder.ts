...
export class SchemaBuilder {
  ...
  /**
   * Builds all schemas reachable from the service's operations.
   *
   ...
   * registry for later protocol error resolution. A minimal constructor that
   * subclasses `Error` is associated so that deserialized errors are throwable.
   */
  private registerError(namespace: string, error: StaticErrorSchema): void {
    const registry = TypeRegistry.for(namespace);
    registry.registerError(error, class extends Error {});
