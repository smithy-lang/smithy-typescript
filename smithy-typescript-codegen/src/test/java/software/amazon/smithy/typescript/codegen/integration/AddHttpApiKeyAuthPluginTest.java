...
public class AddHttpApiKeyAuthPluginTest {
  ...
  public void httpApiKeyAuthClientOnService() {
    testInjects(
        "http-api-key-auth-trait.smithy", "in: 'header', name: 'Authorization', scheme: 'ApiKey'");
  }
  ...
  private void testInjects(String filename, String extra) {
    ...
    assertThat(
        manifest
            .getFileString(CodegenUtils.SOURCE_FOLDER + "/middleware/HttpApiKeyAuth/index.ts")
            .get(),
  ...
  private void testDoesNotInject(String filename) {
    ...
    assertThat(
        manifest.hasFile(CodegenUtils.SOURCE_FOLDER + "/middleware/HttpApiKeyAuth/index.ts"),
        is(false));
