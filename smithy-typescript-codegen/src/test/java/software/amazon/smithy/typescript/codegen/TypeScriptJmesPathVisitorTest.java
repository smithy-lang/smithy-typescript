...
public class TypeScriptJmesPathVisitorTest {
  ...
  public void createsDoubleLengthChecksProfile() {
    String result =
        generateTypescriptInterpretation(
  ...
  public void createComplexLengthFilterContainsIndex() {
    String result =
        generateTypescriptInterpretation(
            "contains(AutoScalingGroups[].[length(Instances[?LifecycleState=='InService']) >="
                + " MinSize][], `false`)");
    assertThat(
        result,
        equalTo(
    ...
    assertThat(
        ...
        equalTo(
            CODEGEN_INDICATOR
                ...
                + " ((result.foo.length > 0.0) || (flat_1.length == 0.0))) ;\n"
                + "}\n"));
  }
  ...
  public void createComplexAndNotIndex() {
    String result =
        generateTypescriptInterpretation(
            "(length(services[?!(length(deployments) == `1` && runningCount == desiredCount)]) =="
                + " `0`)");
    assertThat(
        result,
        equalTo(
            CODEGEN_INDICATOR
                ...
                + "    return (!((element_1.deployments.length == 1.0) && (element_1.runningCount"
                + " == element_1.desiredCount)));\n"
                + "  });\n"
