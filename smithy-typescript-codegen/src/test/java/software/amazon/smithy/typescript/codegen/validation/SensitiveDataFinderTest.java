...
public class SensitiveDataFinderTest {
  StringShape sensitiveString =
      StringShape.builder().addTrait(new SensitiveTrait()).id("foo.bar#sensitiveString").build();

  StringShape dullString = StringShape.builder().id("foo.bar#dullString").build();

  MemberShape memberWithSensitiveData =
      MemberShape.builder().id("foo.bar#sensitive$member").target(sensitiveString.getId()).build();

  MemberShape memberWithDullData =
      MemberShape.builder().id("foo.bar#dull$member").target(dullString.getId()).build();

  MemberShape listMemberWithSensitiveData =
      MemberShape.builder()
          .id("foo.bar#listSensitive$member")
          .target(sensitiveString.getId())
          .build();

  MemberShape listMemberWithDullData =
      MemberShape.builder().id("foo.bar#listDull$member").target(dullString.getId()).build();

  MemberShape mapMemberWithSensitiveKeyData =
      MemberShape.builder()
          .id("foo.bar#mapSensitiveKey$member")
          .target(sensitiveString.getId())
          .build();

  MemberShape mapMemberWithSensitiveValueData =
      MemberShape.builder()
          .id("foo.bar#mapSensitiveValue$member")
          .target(sensitiveString.getId())
          .build();

  StructureShape structureShapeSensitive =
      StructureShape.builder()
          .id("foo.bar#sensitive")
          .members(Collections.singleton(memberWithSensitiveData))
          .build();

  StructureShape structureShapeDull =
      StructureShape.builder()
          .id("foo.bar#dull")
          .members(Collections.singleton(memberWithDullData))
          .build();

  CollectionShape collectionSensitive =
      ListShape.builder()
          .id("foo.bar#listSensitive")
          .addMember(listMemberWithSensitiveData)
          .build();

  CollectionShape collectionDull =
      ListShape.builder().id("foo.bar#listDull").addMember(listMemberWithDullData).build();

  MapShape mapSensitiveKey =
      MapShape.builder()
  ...
  MapShape mapDull =
      MapShape.builder()
          ...
          .value(
              MemberShape.builder().id("foo.bar#mapDull$value").target(dullString.getId()).build())
          .build();
  ...
  MapShape nested2 =
      MapShape.builder()
          ...
          .key(
              MemberShape.builder().id("foo.bar#mapNested2$key").target(dullString.getId()).build())
          .value(
              MemberShape.builder()
