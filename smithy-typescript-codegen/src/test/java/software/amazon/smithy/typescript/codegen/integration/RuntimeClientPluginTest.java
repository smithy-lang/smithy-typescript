...
public class RuntimeClientPluginTest {
  ...
  public void allowsConfigurableSettingsPredicate() {
    ...
    RuntimeClientPlugin createDefaultReadmeFlagPlugin =
        RuntimeClientPlugin.builder()
    ...
    assertThat(
        createDefaultReadmeFlagPlugin.matchesSettings(
            model, service, createDefaultReadmeTrueSettings),
        equalTo(true));
  ...
  public void configuresWithDefaultConventions() {
    Map<String, Object> resolveFunctionParams =
        new HashMap<String, Object>() {
          {
            put("resolveFunctionParam", "resolveFunctionParam");
          }
        };
    Map<String, Object> pluginFunctionParams =
        new HashMap<String, Object>() {
          {
    ...
    assertThat(
        plugin.getResolvedConfig().get().getSymbol().getName(), equalTo("FooResolvedConfig"));

    assertThat(plugin.getResolveFunction().get().getSymbol().getNamespace(), equalTo("foo/baz"));
    assertThat(
        plugin.getResolveFunction().get().getSymbol().getName(), equalTo("resolveFooConfig"));

    assertThat(
        plugin.getAdditionalResolveFunctionParameters(null, null, null),
    ...
    assertThat(
        plugin.getAdditionalPluginFunctionParameters(null, null, null),
    ...
    assertThat(
        plugin.getInputConfig().get().getSymbol().getDependencies().get(0).getVersion(),
        equalTo("1.0.0"));
    assertThat(
  ...
  public void destroyFunctionRequiresResolvedConfig() {
    Assertions.assertThrows(
        ...
        () ->
            RuntimeClientPlugin.builder()
                .withConventions(
                    "foo/baz", "1.0.0", "Foo", RuntimeClientPlugin.Convention.HAS_DESTROY)
                .build());
  }
  ...
  public void convertsToBuilder() {
    RuntimeClientPlugin plugin =
        RuntimeClientPlugin.builder().withConventions("foo/baz", "1.0.0", "Foo").build();
