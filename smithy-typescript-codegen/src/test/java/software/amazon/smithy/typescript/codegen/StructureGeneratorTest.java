...
public class StructureGeneratorTest {
  ...
  private String testStructureCodegenBase(
    ...
    String contents = "";
    contents +=
        manifest.getFileString(CodegenUtils.SOURCE_FOLDER + "//models/models_0.ts").orElse("");
    contents += manifest.getFileString(CodegenUtils.SOURCE_FOLDER + "//models/enums.ts").orElse("");
    contents +=
        manifest.getFileString(CodegenUtils.SOURCE_FOLDER + "//models/errors.ts").orElse("");
  ...
  public void generatesNonErrorStructuresThatExtendOtherInterfaces() {
    ...
    OperationShape operation =
        OperationShape.builder().id("com.foo#Operation").output(struct).build();
