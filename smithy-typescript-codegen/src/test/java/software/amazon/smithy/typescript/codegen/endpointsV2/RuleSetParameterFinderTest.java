...
class RuleSetParameterFinderTest {
  ...
  @Test
  void getEffectiveParams(
      @Mock ServiceShape serviceShape, @Mock EndpointRuleSetTrait endpointRuleSetTrait) {
    EndpointRuleSet endpointRuleSet = EndpointRuleSet.fromNode(ruleSet);
    when(serviceShape.getTrait(EndpointRuleSetTrait.class))
        .thenReturn(Optional.of(endpointRuleSetTrait));
  ...
  void getJmesPathExpression(
    ...
    assertEquals(
        ...
        subject.getJmesPathExpression(
            "?.",
            "input",
            "TransactItems[*].[ConditionCheck.TableName, Put.TableName, Delete.TableName,"
                + " Update.TableName][]"));
  }
}
