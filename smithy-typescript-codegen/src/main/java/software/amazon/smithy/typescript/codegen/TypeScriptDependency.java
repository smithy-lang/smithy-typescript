...
public enum TypeScriptDependency implements Dependency {
  ...
  AWS_SDK_EVENTSTREAM_SERDE_CONFIG_RESOLVER(
      "dependencies", "@smithy/eventstream-serde-config-resolver", false),
  ...
  // This package should never have a major version, and should only use minor and patch versions in
  // development.
  ...
  /**
   * Get all dependencies that are always added to the generated package.json file.
   *
  ...
  /**
   * Creates a Symbol from the dependency of the enum, using the package name and version of the
   * dependency and the provided {@code name}.
   *
   * <p>The created Symbol will have a dependency on the enum's dependency.
   *
  ...
  /**
   * Reads the versions of AWS-published libraries from smithy-aws-typescript-codegen, if it's
            ...
            (k, v) -> {
              if (versions.put(k.toString(), v.toString()) != null) {
                throw new IllegalArgumentException(
                    String.format("Multiple versions defined for %s", k));
              }
            });
