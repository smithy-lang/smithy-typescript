 ...
 *
 * <p>Integrations may mutate this index to customize {@link HttpAuthScheme} implementations.
 */
@SmithyInternalApi
public final class SupportedHttpAuthSchemesIndex {
  ...
  public SupportedHttpAuthSchemesIndex(
      List<TypeScriptIntegration> integrations, Model model, TypeScriptSettings settings) {
    for (TypeScriptIntegration integration : integrations) {
      if (!(integration instanceof HttpAuthTypeScriptIntegration)) {
        continue;
      }
      HttpAuthTypeScriptIntegration httpAuthIntegration =
          (HttpAuthTypeScriptIntegration) integration;
