...
class WaiterGenerator implements Runnable {
  ...
  static final TypeScriptDependency WAITABLE_UTIL_PACKAGE =
      TypeScriptDependency.AWS_SDK_UTIL_WAITERS;
  ...
  public static String getOutputFileLocation(String waiterName) {
    return Paths.get(CodegenUtils.SOURCE_FOLDER, WAITERS_FOLDER, "waitFor" + waiterName + ".ts")
        .toString();
  }
    ...
    writer.openBlock(
        ...
        () -> {
          writer.write(
              ...
              waiter.getMaxDelay());
          writer.write(
              "const result = await createWaiter({...serviceDefaults, ...params}, input,"
                  + " checkState);");
        ...
        .forEach(
            (Acceptor acceptor) -> {
              if (acceptor.getMatcher() instanceof Matcher.SuccessMember) {
                ...
                if (!isException) {
                  Matcher.InputOutputMember member =
                      (Matcher.InputOutputMember) acceptor.getMatcher();
  ...
  private void generateErrorMatcher(
      String accessor, Matcher.ErrorTypeMember member, AcceptorState state) {
    writer.openBlock(
        "if ($L.name && $L.name == $S) {",
        "}",
        accessor,
  ...
  private void generatePathMatcher(String accessor, PathMatcher pathMatcher, AcceptorState state) {
    writer.openBlock(
        ...
        () -> {
          ...
          TypeScriptJmesPathVisitor expressionVisitor =
              new TypeScriptJmesPathVisitor(writer, accessor, expression);
          String expectedState = makeWaiterResult(state);
          expressionVisitor.run();

          switch (pathMatcher.getComparator()) {
            case ALL_STRING_EQUALS:
              expressionVisitor.writeAllStringEqualsExpectation(
                  pathMatcher.getExpected(), expectedState);
              break;
            case ANY_STRING_EQUALS:
              expressionVisitor.writeAnyStringEqualsExpectation(
                  pathMatcher.getExpected(), expectedState);
              break;
            case STRING_EQUALS:
  ...
  private String makeWaiterResult(AcceptorState resultantState) {
    ...
    throw new CodegenException(
        "Hit an invalid acceptor state to codegen " + resultantState.toString());
  }
