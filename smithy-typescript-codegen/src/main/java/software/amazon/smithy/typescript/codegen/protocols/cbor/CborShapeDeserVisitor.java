...
public class CborShapeDeserVisitor extends DocumentShapeDeserVisitor {
  ...
  @Override
  protected void deserializeCollection(
            ...
            if (filterExpression.isEmpty()) {
              writer.openBlock(
                  ...
                  () -> {
                    if (!shape.hasTrait(SparseTrait.ID)) {
                      writer.write(
                          "throw new TypeError('All elements of the non-sparse list $S must be"
                              + " non-null.');",
                          shape.getId());
                    } else {
    ...
    writer.openBlock(
        ...
        () -> {
          ...
          if (shape.hasTrait(SparseTrait.ID)) {
            ...
            writer
                .write("acc[key as $T] = null as any;", symbolProvider.toSymbol(shape.getKey()))
                .dedent();
            writer.write("}");
          }
  ...
  @Override
  protected void deserializeStructure(
              ...
              (memberName, memberShape) -> {
                ...
                if (usesExpect(target)) {
                  ...
                  if (valueExpression.equals(propertyAccess)) {
                    ...
                    if (isUnaryCall) {
                      writer.write(
                          "'$1L': $2L,", memberName, UnaryFunctionCall.toRef(functionExpression));
                    } else {
  ...
  protected void deserializeUnion(ProtocolGenerator.GenerationContext context, UnionShape shape) {
    ...
    members.forEach(
        (memberName, memberShape) -> {
          ...
          String memberValue =
              target.accept(getMemberVisitor(PropertyAccessor.getFrom("output", memberName)));
