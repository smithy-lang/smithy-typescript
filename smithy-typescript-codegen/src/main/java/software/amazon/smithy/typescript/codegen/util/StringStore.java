...
/**
 * Intended for use at the {@link
 * software.amazon.smithy.typescript.codegen.integration.ProtocolGenerator.GenerationContext} level,
 * this class allocates and tracks variables assigned to string literals, allowing a form of
 * compression on long protocol serde files.
 */
@SmithyInternalApi
public class StringStore {
  /** Words are the component strings found within `camelCaseWords` or `header-dashed-words`. */
  private static final Pattern FIND_WORDS =
      Pattern.compile("(x-amz)|(-\\w{3,})|(^[a-z]{3,})|([A-Z][a-z]{2,})");
  ...
  public String var(String literal, String preferredPrefix) {
    Objects.requireNonNull(literal);
    return literalToVariable.computeIfAbsent(
        literal, (String key) -> assignPreferredKey(key, preferredPrefix));
  }
  ...
  /**
   * Outputs the generated code for any constants that have been allocated but not yet retrieved.
   */
  ...
  /**
   * Assigns a new variable for a given string literal. Avoid calling assignKey more than once for a
   * given literal, for example with {@link HashMap#computeIfAbsent(Object, Function)}, since it
   * would allocate two different variables.
   */
  ...
  /**
   * Assigns a unique variable using the letters from the literal. Prefers the uppercase or
   * word-starting letters.
   */
  private String allocateVariable(String literal) {
    String[] sections =
        Arrays.stream(literal.split("[-_\\s]")).filter(s -> !s.isEmpty()).toArray(String[]::new);
