...
final class PackageJsonGenerator {
  ...
  static void writePackageJson(
    ...
    ObjectNode mergedScripts =
        defaultPackageJson
            .expectObjectMember("scripts")
            .merge(
                userSuppliedPackageJson
                    .getObjectMember("scripts")
                    .orElse(ObjectNode.builder().build()));

    ObjectNode node =
        defaultPackageJson.merge(userSuppliedPackageJson).withMember("scripts", mergedScripts);
    ...
    // Add test script and vitest.config.mts if specs and their devDependency on vitest has been
    // generated.
    ...
    if (settings.generateTypeDoc()) {
      ...
      if (devDeps.getMember(TypeScriptDependency.TYPEDOC.packageName).isEmpty()) {
        devDeps =
            devDeps.withMember(
                TypeScriptDependency.TYPEDOC.packageName, TypeScriptDependency.TYPEDOC.version);
    ...
    if (settings.generateClient()) {
      ...
      node =
          node.withMember(
              "react-native",
              node.getObjectMember("react-native")
                  .orElse(Node.objectNode())
    ...
    if (!settings.generateIndexTests()) {
      node =
          node.withMember(
              "scripts", node.getObjectMember("scripts").get().withoutMember("test:index"));
    }
