 ...
 *
 * <p>Use the {@code $T} formatter to refer to {@link Symbol}s. These symbols are automatically
 * imported into the writer and relativized if necessary.
 *
 * <p>When adding imports, start the module name with "./" to resolve relative module paths against
 * the moduleName of the writer. Module names that start with anything other than "." (e.g., "@",
 * "/", etc.) are never relativized.
 *
 * <p>Dependencies introduced via a TypeScriptWriter are added to the package.json file if the
 * writer is a part of the {@link TypeScriptDelegator} of the {@link DirectedTypeScriptCodegen}.
 */
@SmithyUnstableApi
public final class TypeScriptWriter extends SymbolWriter<TypeScriptWriter, ImportDeclarations> {
  ...
  public static final class TypeScriptWriterFactory
      implements SymbolWriter.Factory<TypeScriptWriter> {

    @Override
    public TypeScriptWriter apply(String filename, String namespace) {
      boolean withAttribution = filename.endsWith(".ts");
      String moduleName =
          filename.endsWith(".ts") ? filename.substring(0, filename.length() - 3) : filename;
   ...
   * @return Returns the writer.
   * @deprecated Use {@link TypeScriptWriter#addImport(String, String, TypeScriptDependency)}
   *     addImport}
   */
  @Deprecated
  ...
  /**
   * Imports a type using an alias from a module only if necessary. Adds the dependency.
   *
   * @param name Type to import.
   * @param as Alias to refer to the type as.
  ...
  /**
   * Same as {@link #addImport(String, String, PackageContainer)} but appends a submodule path, for
   * example "@smithy/core/cbor".
   */
  public TypeScriptWriter addImportSubmodule(
      String name, String as, PackageContainer from, String submodule) {
  ...
  public TypeScriptWriter addTypeImportSubmodule(
      String name, String as, PackageContainer from, String submodule) {
    if (from instanceof Dependency dependency) {
      addDependency(dependency);
    }
  ...
  /**
   * As openBlock, but collapses all space between open and close strings if the condition is not
   * met.
   */
  public TypeScriptWriter openCollapsibleBlock(
      String open, String close, boolean condition, Object[] args, Runnable runnable) {
    if (condition) {
  ...
  public TypeScriptWriter openCollapsibleBlock(
      String open, String close, boolean condition, Runnable runnable) {
    return openCollapsibleBlock(open, close, condition, new Object[] {}, runnable);
  }

  public TypeScriptWriter openCollapsibleBlock(
      String open, String close, boolean condition, Object arg1, Runnable runnable) {
  ...
  /**
   * This is private because the string-value [from] signature is deprecated for the corresponding
   * addImport() signature. Importing a string makes it difficult to ensure the package is added to
   * the dependency manifest, so package imports should be by package object, and relative imports
   * by path object.
   */
  ...
  /**
   * Check that the import source string has been registered with the dependency manifest if it is a
   * package, not a node package, and not a relative path.
   */
  ...
  private final class TypeScriptSymbolFormatter implements BiFunction<Object, String, String> {
    ...
    public String apply(Object type, String indent) {
      if (type instanceof Symbol) {
        ...
        throw new CodegenException(
            "Invalid type provided to $T. Expected a Symbol or SymbolReference, but found `"
                + type
                + "`");
      }
    }
