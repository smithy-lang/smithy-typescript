...
/**
 * Java SPI for customizing TypeScript code generation, registering new protocol code generators,
 * renaming shapes, modifying the model, adding custom code, etc.
 */
@SmithyUnstableApi
public interface TypeScriptIntegration
   ...
   *
   * <p>This is annotated as a Smithy Internal API, and may be removed at any point.
   *
   ...
   *
   * <p>Implementations of this method are expected to add fields to the "ClientDefaults" interface
   * of a generated client. This interface contains fields that are either statically generated from
   * a model or are dependent on the runtime that a client is running in. Implementations are
   * expected to write interface field names and their type signatures, each followed by a semicolon
   * (;). Any number of fields can be added, and any {@link Symbol} or {@link SymbolReference}
   * objects that are written to the writer are automatically imported, and any of their contained
   * {@link SymbolDependency} values are automatically added to the generated {@code package.json}
   * file.
   *
   ...
   *
   * <p>Implementations of this method are expected to add values to a runtime-specific or shared
   * configuration object that is used to provide values for a "ClientDefaults" interface. This
   * method is invoked for every supported {@link LanguageTarget}. Implementations are expected to
   * branch on the provided {@code LanguageTarget} and add the appropriate default values and
   * imports, each followed by a (,). Any number of key-value pairs can be added, and any {@link
   * Symbol} or {@link SymbolReference} objects that are written to the writer are automatically
   * imported, and any of their contained {@link SymbolDependency} values are automatically added to
   * the generated {@code package.json} file.
   *
   * <p>For example, the following code adds two values for both the node and browser targets and
   * ignores the SHARED target:
   *
   * <pre>{@code
   * public final class MyIntegration implements TypeScriptIntegration {
   *
   ...
   * }</pre>
   *
   * <p>The following code adds a value to the runtimeConfig.shared.ts file so that it used on all
   * platforms. It pulls a trait value from the service being generated and adds it to the client
   * configuration. Note that a corresponding entry needs to be added to {@link
   * #addConfigInterfaceFields} to make TypeScript aware of the property.
   *
   * <pre>{@code
   ...
   * @return Returns a map of config property name and a consumer function with TypeScriptWriter
   *     parameter.
   */
   ...
   *
   * <p>A client configuration interface contains settings that modify a service client. The client
   * configuration interface enables configuring timeouts, retry strategy, etc for the client.
   *
   * <p>Multiple interfaces are used to define the client configuration. For example:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * During code-generation, smithy-typescript will aggregate the interfaces and create a single
  ...
  /**
   * Allows the customization to write arbitrary preparatory code prior to the returned config
   * object.
   */
  @SmithyInternalApi
  default void prepareCustomizations(
      TypeScriptWriter writer, LanguageTarget target, TypeScriptSettings settings, Model model) {
    return;
  }
}
