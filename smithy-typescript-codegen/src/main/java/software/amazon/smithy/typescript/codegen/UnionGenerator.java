 ...
 *
 * <p>Smithy tagged unions are rendered as a set of TypeScript interfaces and functionality used to
 * visit each variant. Only a single member can be set at any given time. A member that contains
 * unknown variants is automatically added to each tagged union. If set, it contains the name of the
 * property that was set and its value stored as an {@code any}.
 *
 * <p>A {@code Visitor} interface and a method used to dispatch to the visitor is generated for each
 * tagged union. This allows for working with tagged unions functionally and account for each
 * variant in a typed way.
 *
 ...
 * }</pre>
 *
 * <p>Important: Tagged unions in TypeScript are intentionally designed so that it is
 * forward-compatible to change a structure with optional and mutually exclusive members to a tagged
 * union.
 */
@SmithyInternalApi
final class UnionGenerator implements Runnable {
  ...
  UnionGenerator(
      Model model, SymbolProvider symbolProvider, TypeScriptWriter writer, UnionShape shape) {
