...
final class ServerCommandGenerator implements Runnable {
  ...
  ServerCommandGenerator(
    ...
    errors =
        Collections.unmodifiableList(operationIndex.getErrors(operation, settings.getService()));
  }
  ...
  private void writeInputType(String typeName, Optional<StructureShape> inputShape) {
    if (inputShape.isPresent()) {
      ...
      writer.write(
          "export interface $L extends $T {}", typeName, symbolProvider.toSymbol(inputShape.get()));
  ...
  private void writeOperationSerializer() {
    ...
    String serializerName =
        operationSymbol.expectProperty("serializerType", Symbol.class).getName();
  ...
  private void writeErrorChecker() {
    writer.openBlock(
        "isOperationError(error: any): error is $T {",
        "};",
        errorsType,
        () -> {
          if (errors.isEmpty()) {
    ...
    writer.openBlock(
        ...
        () -> {
          if (protocolGenerator == null) {
            ...
            String serializerFunction =
                ProtocolGenerator.getGenericSerFunctionName(errorSymbol) + "Error";
            writer.addRelativeImport(
                serializerFunction,
                null,
                Paths.get(
