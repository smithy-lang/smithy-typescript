...
public final class SmithyProtocolUtils {
  ...
  public static void generateDocumentBodyShapeSerde(
      ProtocolGenerator.GenerationContext context, Set<Shape> shapes, ShapeVisitor<Void> visitor) {
  ...
  public static void generateProtocolTests(
      ProtocolGenerator generator, ProtocolGenerator.GenerationContext context) {
  ...
  private static boolean filterMalformedRequestTests(
    ...
    // but not others.
    if (operation.getId().getName().equals("MalformedLong")
        && testCase.hasTag("underflow/overflow")) {
      return true;
    }
