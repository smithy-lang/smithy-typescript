...
final class ImportDeclarations implements ImportContainer {
    ...
    if (!defaultImports.isEmpty()) {
      for (Map.Entry<String, Pair<String, Ignore>> importEntry : defaultImports.entrySet()) {
        ...
        if (ignore) {
          result
              .append("// @ts-ignore: ")
              .append(importEntry.getValue().getRight().reason)
              .append("\n");
        }
        result
            .append("import ")
  ...
  private static void createImports(
    ...
    TreeSet<String> mergedModuleKeys =
        new TreeSet<>(
            (a, b) -> {
    ...
    long separatorIndex = mergedModuleKeys.stream().filter(k -> !k.startsWith(".")).count();
    int i = 0;
    ...
    for (String module : mergedModuleKeys) {
      ...
      Map<String, String> typeImports =
          namedTypeImports.getOrDefault(module, Collections.emptyMap());
      ...
      for (String alias : mergedSymbolKeys) {
        ...
        if ("*".equals(runtimeSymbol) || "*".equals(typeSymbol)) {
          throw new CodegenException(
              "Star imports are not supported, attempted for "
                  + module
  ...
  private static String getRelativizedModule(String relativize, String module) {
    if (relativize != null && module.startsWith(".")) {
      ...
      module =
          relativizePath.relativize(Paths.get(module)).toString().replace(File.separatorChar, '/');
