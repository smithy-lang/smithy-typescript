...
public class SchemaGenerator implements Runnable {
  ...
  /**
   * @return variable name of the shape's schema, with deconfliction for multiple namespaces with
  ...
  /**
   * Writes the schema declaration for a simple shape. If it has no runtime traits, e.g. a plain
   * string, nothing will be written.
   */
        ...
        () -> {
          ...
          if (shape.hasTrait(ErrorTrait.class)) {
            ...
            writer.addRelativeImport(
                symbolName, exceptionCtorSymbolName, Paths.get("..", "models", "errors"));
            writer.openBlock(
                """
            ...
            writer.addImportSubmodule(
                "TypeRegistry", null, TypeScriptDependency.SMITHY_CORE, "/schema");
            writer.write(
                """
  ...
  private void checkedWriteSchema(Shape shape, Runnable schemaWriteFn) {
    if (shape.getId().getNamespace().equals("smithy.api")
      ...
      String sentinel =
          this.resolveSchema(model.expectShape(ShapeId.from("smithy.api#Unit")), shape);
  ...
  /**
   * @return generally the symbol name of the target shape, but sometimes a sentinel value for
   *     special types like blob and timestamp.
   */
   ...
   * @throws IllegalArgumentException when no sentinel value exists, e.g. a non-simple schema was
   *     passed in.
   */
  ...
  /**
   * For example, the number 5 represents a timestamp (Date-Time) schema with no other traits. For
   * lists, the bit modifier 64 is applied, giving 64 | 5 for a list of timestamps. For further
   * nested containers, bit masks can no longer be used, necessitating the `sim` simple schema
