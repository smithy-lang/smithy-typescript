...
public final class AddBaseServiceExceptionClass implements TypeScriptIntegration {
  ...
  private void writeAdditionalFiles(
    ...
    if (isClientSdk) {
      ...
      writerFactory.accept(
          ...
          writer -> {
            writer.addImport(
                "ServiceException", "__ServiceException", TypeScriptDependency.AWS_SMITHY_CLIENT);
  ...
  /**
   * For any error shape, add the reference of the base error class to the error symbol's
   * references. In client SDK, the base error class is the service-specific service exception
   * class. In server SDK, the base error class is the ServiceException class from server-common
   * package.
   */
  @Override
