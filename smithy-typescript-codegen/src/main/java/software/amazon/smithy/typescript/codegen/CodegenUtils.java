...
public final class CodegenUtils {
  ...
  public static String getOperationDeserializerContextType(
      TypeScriptSettings settings, TypeScriptWriter writer, Model model, OperationShape operation) {
    // Get default SerdeContext.
    List<String> contextInterfaceList = getDefaultOperationSerdeContextTypes(writer);
    // If event stream trait exists, add corresponding serde context type to the intersection type.
    EventStreamIndex eventStreamIndex = EventStreamIndex.of(model);
    ...
    if (AddSdkStreamMixinDependency.hasStreamingBlobDeser(settings, model, operation)) {
      writer.addTypeImport(
          "SdkStreamSerdeContext", "__SdkStreamSerdeContext", TypeScriptDependency.SMITHY_TYPES);
  ...
  /**
   * Generate the type of the command input of the client sdk given the streaming blob member of the
   * shape. The generated type eases the streaming member requirement so that users don't need to
   * construct a stream every time. This type decoration is allowed in Smithy because it makes, for
   * the same member, the type to be serialized is more permissive than the type to be deserialized.
  ...
  /**
   * Generate the type of the command output of the client sdk given the streaming blob member of
   * the shape. The type marks the streaming blob member to contain the utility methods to transform
   * the stream to string, buffer or WHATWG stream API.
   */
  static void writeClientCommandStreamingOutputType(
    ...
    writer.addTypeImport(
        "StreamingBlobPayloadOutputTypes", null, TypeScriptDependency.SMITHY_TYPES);
  ...
  /**
   * Returns the list of function parameter key-value pairs to be written for provided parameters
   * map.
   *
  ...
  static List<String> getFunctionParametersList(Map<String, Object> paramsMap) {
    ...
    List<Map.Entry<String, Object>> sortedParamsMap =
        paramsMap.entrySet().stream().sorted(Map.Entry.comparingByKey()).toList();
      ...
      for (Map.Entry<String, Object> param : sortedParamsMap) {
        ...
        if (value instanceof Symbol) {
          ...
          functionParametersList.add(
              String.format(
                  "'%s': [%s]", key, valueStringList.stream().collect(Collectors.joining(", "))));
        } else if (value instanceof Map) {
          Map<?, ?> valueMap = (Map<?, ?>) value;
          if (!valueMap.isEmpty()
              && valueMap.keySet().stream().anyMatch(k -> !(k instanceof String))
              && valueMap.values().stream().anyMatch(v -> !(v instanceof String))) {
            throw new CodegenException(
                "Plugin function parameters map must be Map<String, String>");
          }
          List<String> valueStringList =
          ...
          throw new CodegenException(
              "Plugin function parameters not supported for type " + value.getClass());
        }
      }
    }
  ...
  /**
   * Ease the input streaming member restriction so that users don't need to construct a stream
   * every time. This is used for inline type declarations (such as parameters) that need to take
   * more permissive inputs Refer here for more rationales:
  ...
  /**
   * The alternative should be used because the base exception may have been renamed due to a
  ...
  /**
   * If the service unfortunately defines a ServiceNameBaseException error, we still generate a
   * synthetic base error for uniformity, but it must be renamed.
   */
  public static String getSyntheticBaseExceptionName(String serviceName, Model model) {
    ...
    while (true) {
      ...
      boolean namingCollision =
          model.getStructureShapes().stream()
              .anyMatch(
                  structureShape ->
                      structureShape.getId().getName().equals(finalServiceExceptionName));

      if (namingCollision) {
        serviceExceptionName =
            serviceExceptionName.replaceAll("ServiceException$", "SyntheticServiceException");
      } else {
        break;
      }
    }
