...
/**
 * Generates document-shape serializers for the Smithy RPCv2 JSON protocol.
 *
 * <p>The structure of these serializers is format-neutral (they build plain JS
 * objects); the format-specific behavior lives in {@link JsonRpcMemberSerVisitor}
 * and in the top-level {@code JSON.stringify} wrapping performed by the generator.
 */
public class JsonRpcShapeSerVisitor extends DocumentShapeSerVisitor {
    ...
    public JsonRpcShapeSerVisitor(ProtocolGenerator.GenerationContext context) {
        super(context);
    ...
    private DocumentMemberSerVisitor getMemberVisitor(String dataSource) {
        return new JsonRpcMemberSerVisitor(getContext(), dataSource);
    }
}
