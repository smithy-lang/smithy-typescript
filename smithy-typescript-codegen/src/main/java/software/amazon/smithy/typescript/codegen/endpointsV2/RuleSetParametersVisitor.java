...
/**
 * Writes endpoint ruleset params into a client-specific config resolver step, applying defaults as
 * needed.
 */
public class RuleSetParametersVisitor extends NodeVisitor.Default<Void> {
  ...
  public RuleSetParametersVisitor(
      TypeScriptWriter writer, Map<String, String> clientContextParams, boolean useLocalNames) {
    this.writer = writer;
  ...
  public Void objectNode(ObjectNode node) {
    ...
    for (Map.Entry<StringNode, Node> entry : members.entrySet()) {
      ...
      ParameterGenerator parameterGenerator =
          new ParameterGenerator(localKey, param, useLocalNames);
