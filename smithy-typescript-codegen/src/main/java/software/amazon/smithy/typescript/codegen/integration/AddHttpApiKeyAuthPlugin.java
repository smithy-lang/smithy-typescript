 ...
 *
 * <p>This is legacy auth behavior, and is no longer in development.
 */
@SmithyInternalApi
public final class AddHttpApiKeyAuthPlugin implements TypeScriptIntegration {
   ...
   *
   * <p>This adds the configuration items to the client config and plugs in the middleware to
   * operations that need it.
   *
   * <p>The middleware will inject the client's configured API key into the request as defined by
   * the @httpApiKeyAuth trait. If the trait says to put the API key into a named header, that
   * header will be used, optionally prefixed with a scheme. If the trait says to put the API key
   * into a named query parameter, that query parameter will be used.
   */
  @Override
            ...
            .additionalPluginFunctionParamsSupplier(
                (m, s, o) ->
                    new HashMap<String, Object>() {
                      {
                        // It's safe to do expectTrait() because the operation predicate ensures
                        // that the trait
                        // exists `in` and `name` are required attributes of the trait, `scheme` is
                        // optional.
  ...
  private void writeAdditionalFiles(
    ...
    writerFactory.accept(
        Paths.get(CodegenUtils.SOURCE_FOLDER, "middleware", INTEGRATION_NAME, "index.ts")
            .toString(),
        writer -> {
  ...
  private void writeAdditionalExports(
      TypeScriptSettings settings, Model model, TypeScriptWriter writer) {
