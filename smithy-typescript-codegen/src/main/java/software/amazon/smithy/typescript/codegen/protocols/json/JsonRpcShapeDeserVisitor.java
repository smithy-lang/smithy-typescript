...
/**
 * Generates document-shape deserializers for the Smithy RPCv2 JSON protocol.
 *
 * <p>The structure of these deserializers is format-neutral; the format-specific
 * behavior lives in {@link JsonRpcMemberDeserVisitor}.
 */
public class JsonRpcShapeDeserVisitor extends DocumentShapeDeserVisitor {

    public JsonRpcShapeDeserVisitor(ProtocolGenerator.GenerationContext context) {
        super(context);
    ...
    private JsonRpcMemberDeserVisitor getMemberVisitor(String dataSource) {
        return new JsonRpcMemberDeserVisitor(getContext(), dataSource);
    }
