...
public class RuleSetParameterFinder {
  ...
  /**
   * It's possible for a parameter to pass validation, i.e. exist in the modeled shapes and be used
   * in endpoint tests, but have no actual effect on endpoint resolution.
   *
  ...
  /**
   * TODO(endpointsv2) From definitions in EndpointRuleSet.parameters, or TODO(endpointsv2) are they
   * from the closed set?
   */
      ...
      clientContextParamsTrait
          ...
          .forEach(
              (name, definition) -> {
                ...
                if (shapeType.isShapeType(ShapeType.STRING)
                    || shapeType.isShapeType(ShapeType.BOOLEAN)) {
                  map.put(
                      name,
                  ...
                  map.put(
                      name, "string[]" // Only string lists are supported.
                      );
                } else {
  ...
  /**
   * The contextParam trait allows for binding a structure's member value to a context parameter
   * name. This trait MUST target a member shape on an operation's input structure. The targeted
   * endpoint parameter MUST be a type that is compatible with member's shape targeted by the trait.
   */
  public Map<String, String> getContextParams(Shape operationInput) {
    ...
    if (operationInput.isStructureShape()) {
      operationInput
          .getAllMembers()
          .forEach(
              (String memberName, MemberShape member) -> {
  ...
  public Map<String, String> getOperationContextParamValues(OperationShape operation) {
    ...
    Optional<OperationContextParamsTrait> trait =
        operation.getTrait(OperationContextParamsTrait.class);
    if (trait.isPresent()) {
      trait
          .get()
          .getParameters()
          .forEach(
              (name, definition) -> {
                ...
                if (value.endsWith(separator + noOpMap)) {
                  value =
                      value.substring(0, value.length() - noOpMap.length() - separator.length());
                }
  ...
  private String getJmesPathExpressionSection(String separator, String value, String part) {
    ...
    // Treat remaining part as identifier without spaces
