...
/**
 * This validator tells you whether a shape contains sensitive data fields. This is used to decide
 * whether a sensitive log filter function needs to be generated for a given shape.
 */
public class SensitiveDataFinder {
  ...
  /**
   * @param model - model context for the {@link #findsSensitiveDataIn(Shape)} queries.
   */
  ...
  private boolean findRecursive(Shape shape) {
    ...
    Selector sensitiveSelector =
        Selector.parse("[id = '" + shape.getId() + "']" + " ~> [trait|sensitive]");
    Selector streamingSelector =
        Selector.parse("[id = '" + shape.getId() + "']" + " ~> [trait|streaming]");
