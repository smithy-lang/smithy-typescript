...
/**
 * Index of ShapeIds to a boolean indicating whether a shape's serde function may be omitted. If the
 * shape is of a certain type, and has no downstream incompatible shapes or traits that require
 * additional handling, its serde function may be emitted.
 */
public class SerdeElisionIndex implements KnowledgeIndex {
  private final Map<ShapeId, Boolean> elisionBinding = new HashMap<>();
  private final Map<String, ShapeId> mutatingTraits =
      MapUtils.of(
  ...
  private boolean hasMutatingTraits(Shape shape, Model model) {
    for (var entry : mutatingTraits.entrySet()) {
      ...
      Selector selector =
          Selector.parse("[id = '" + shape.getId() + "']" + " ~> [trait|" + entry.getKey() + "]");
  ...
  private boolean hasIncompatibleTypes(Shape shape, Model model, int depth) {
    ...
    switch (target.getType()) {
      ...
      case STRUCTURE:
        return target.asStructureShape().get().getAllMembers().values().stream()
            .anyMatch(s -> hasIncompatibleTypes(s, model, depth + 1));
      case UNION:
        return target.asUnionShape().get().getAllMembers().values().stream()
            .anyMatch(s -> hasIncompatibleTypes(s, model, depth + 1));
      case MAP:
