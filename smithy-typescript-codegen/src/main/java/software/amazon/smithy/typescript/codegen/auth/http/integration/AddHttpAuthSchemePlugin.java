...
public final class AddHttpAuthSchemePlugin implements HttpAuthTypeScriptIntegration {
  ...
  public List<RuntimeClientPlugin> getClientPlugins() {
    Map<String, ClientWriterConsumer> httpAuthSchemeParametersProvider =
        Map.of(
  ...
  public void customize(TypeScriptCodegenContext codegenContext) {
    ...
    codegenContext
        .writerDelegator()
        .useFileWriter(
            AuthUtils.HTTP_AUTH_SCHEME_PROVIDER_PATH,
            w -> {
              SupportedHttpAuthSchemesIndex authIndex =
                  new SupportedHttpAuthSchemesIndex(
              ...
              Map<ShapeId, HttpAuthScheme> httpAuthSchemes =
                  AuthUtils.getAllEffectiveNoAuthAwareAuthSchemes(
                      serviceShape, serviceIndex, authIndex, topDownIndex);
   ...
   * Example: ```typescript defaultWeatherHttpAuthSchemeParametersProvider; ```
   */
  private static void httpAuthSchemeParametersProvider(
      TypeScriptWriter w, ClientBodyExtraCodeSection clientBodySection) {
    String httpAuthSchemeParametersProviderName =
        "default"
            ...
            + "HttpAuthSchemeParametersProvider";
    w.addImport(
        httpAuthSchemeParametersProviderName, null, AuthUtils.AUTH_HTTP_PROVIDER_DEPENDENCY);
   ...
   * Example: ```typescript async (config: WeatherClientResolvedConfig) => new
   * DefaultIdentityProviderConfig({ "aws.auth#sigv4": config.credentials,
   * "smithy.api#httpApiKeyAuth": config.apiKey, "smithy.api#httpBearerAuth": config.token, }) ```
   */
  private static void identityProviderConfigProvider(
      TypeScriptWriter w, ClientBodyExtraCodeSection s) {
    ...
    SupportedHttpAuthSchemesIndex authIndex =
        new SupportedHttpAuthSchemesIndex(s.getIntegrations(), s.getModel(), s.getSettings());
          ...
          for (HttpAuthScheme scheme : httpAuthSchemes.values()) {
            ...
            for (ConfigField configField : scheme.getConfigFields()) {
              if (configField.type().equals(ConfigField.Type.MAIN)) {
                w.writeInline(
                    "$S: config.$L,", scheme.getSchemeId().toString(), configField.name());
              }
            }
          }
        });
  ...
  */
  private void generateHttpAuthSchemeInputConfigInterface(
      TypeScriptWriter w, HttpAuthSchemeInputConfigInterfaceCodeSection s) {
    w.pushState(s);
  ...
  */
  private void generateHttpAuthSchemeResolvedConfigInterface(
      TypeScriptWriter w, HttpAuthSchemeResolvedConfigInterfaceCodeSection s) {
    w.pushState(s);
    Map<String, ConfigField> configFields = s.getConfigFields();
    Map<Symbol, ResolveConfigFunction> resolveConfigFunctions = s.getResolveConfigFunctions();
    String serviceName =
  ...
  */
  private void generateResolveHttpAuthSchemeConfigFunction(
      TypeScriptWriter w, ResolveHttpAuthSchemeConfigFunctionCodeSection s) {
    w.pushState(s);
    Map<String, ConfigField> configFields = s.getConfigFields();
    Map<Symbol, ResolveConfigFunction> resolveConfigFunctions = s.getResolveConfigFunctions();
    Map<Symbol, ResolveConfigFunction> previousResolvedFunctions =
        resolveConfigFunctions.entrySet().stream()
