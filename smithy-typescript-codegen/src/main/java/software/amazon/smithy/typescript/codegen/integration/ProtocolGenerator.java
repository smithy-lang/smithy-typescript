...
public interface ProtocolGenerator {
   ...
   *
   * <p>For example, the default implementation converts "." to "_", and converts "-" to become
   ...
   *
   * <p>The default implementation is the ShapeId name of the protocol trait in Smithy models (e.g.,
  ...
  /**
   * Determines if two protocol generators are compatible at the application protocol level, meaning
   * they both use HTTP, or MQTT for example.
   *
   * <p>Two protocol implementations are considered compatible if the {@link
   * ApplicationProtocol#equals} method of {@link #getApplicationProtocol} returns true when called
   * with {@code other}. The default implementation should work for most interfaces, but may be
   * overridden for more in-depth handling of things like minor version incompatibilities.
   *
   * <p>By default, if the application protocols are considered equal, then {@code other} is
   * returned.
   *
      ...
      throw new CodegenException(
          String.format(
              "All of the protocols generated for a service must be runtime compatible, but"
                  + " protocol `%s` is incompatible with other application protocols: [%s]. Please"
                  + " pick a set of compatible protocols using the `protocols` option when"
                  + " generating %s.",
  ...
  /**
   * Generates the code used to handle a request for a specific operation in the given service. This
   * allows the business logic for a service to be split among multiple deployment targets, for
   * example, one Lambda function per operation.
   *
   * @param context Generation context.
  ...
  static String getSerFunctionShortName(Symbol symbol) {
    ...
    // Update the function to have a component based on the symbol.
    functionName +=
        ProtocolGenerator.getSerdeFunctionSymbolComponent(
            symbol, symbol.expectProperty("shape", Shape.class));
    return functionName;
  }
   ...
   */
  default Map<String, ShapeId> getOperationErrors(
  ...
  /**
   * @return map of fully qualified shape id to aliases and/or short names that should map to the
   *     same error.
   */
  ...
  class GenerationContext {
    ...
    // where the constructor asserts that both writer/writerDelegator are not set, instead of
    // unsetting the other.
    // Or consider specialized GenerationContextWithWriter/GenerationContextWithWriterDelegator to
    // use in
    // corresponding ProtocolGenerator methods that need writer v/s writerDelegator.
    // OR better refactor to add something like a `areProtocolTestsPresent()` so
    // `generateProtocolTests` is called
