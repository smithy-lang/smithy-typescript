 ...
 *
 * <ul>
 *   <li>Blob: base64 encoded.
 *   <li>BigInteger, BigDecimal: converted to strings to maintain precision.
 *   <li>Timestamp: converted to a representation based on the specified format.
 *   <li>Service, Operation, Resource, Member: not serializable in documents. <b>Not
 *       overridable.</b>
 *   <li>Document, List, Map, Set, Structure, Union: delegated to a serialization function. <b>Not
 *       overridable.</b>
...
public class DocumentMemberSerVisitor implements ShapeVisitor<String> {
   ...
   *     such as @xmlName. Can be, and defaults, to, null.
   */
  ...
  /**
   * Gets the in-code location of the data to provide an input of ({@code input.foo}, {@code entry},
   * etc.).
   *
  ...
  private String handleFloat() {
    context
        .getWriter()
        .addImport("serializeFloat", "__serializeFloat", TypeScriptDependency.AWS_SMITHY_CLIENT);
  ...
  /** This should not be called, since big number serde is handled by format specific visitors. */
  @Override
  public String bigIntegerShape(BigIntegerShape shape) {
    if (context.getSettings() != null
        && context.getSettings().getBigNumberMode().equals("big.js")) {
  ...
  public String timestampShape(TimestampShape shape) {
    ...
    Format format =
        httpIndex.determineTimestampFormat(shape, Location.DOCUMENT, defaultTimestampFormat);
