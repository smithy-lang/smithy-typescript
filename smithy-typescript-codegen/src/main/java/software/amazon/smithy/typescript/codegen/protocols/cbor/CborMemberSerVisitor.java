...
public class CborMemberSerVisitor extends DocumentMemberSerVisitor {
  ...
  /**
   * This differs from the base method in that CBOR does not need to wrap the blob value in
   * `context.base64Encoder(...)`. The CBOR format serializer already does this whereas e.g.
  ...
  /**
   * +/- Infinity and NaN have byte representations. No need to serialize those values with
   * serializeFloat().
   */
  @Override
  ...
  public String bigDecimalShape(BigDecimalShape shape) {
    context
        .getWriter()
        .addImportSubmodule("nv", "__nv", TypeScriptDependency.SMITHY_CORE, "/serde");
