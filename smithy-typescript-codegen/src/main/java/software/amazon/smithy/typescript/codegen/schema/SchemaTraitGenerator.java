...
/**
 * Creates the string representing a trait's data. For presence-based trait, essentially boolean, a
 * 1 or 2 will be used.
 */
@SmithyInternalApi
public class SchemaTraitGenerator {
  private static final String ANNOTATION_TRAIT_VALUE = "1";
  private static final Set<ShapeId> ANNOTATION_TRAITS =
      SetUtils.of(
  ...
  /**
   * Data traits are traits with one or more fields of data. To allow for the possibility of the
   * traits adding new fields, the generated schema object MUST be an array with consistent ordering
   * and size for the fields' data.
   */
  private static final Set<ShapeId> DATA_TRAITS =
      SetUtils.of(HttpErrorTrait.ID, HttpTrait.ID, EndpointTrait.ID, XmlNamespaceTrait.ID);

  private static final Set<ShapeId> STRING_TRAITS =
      SetUtils.of(
