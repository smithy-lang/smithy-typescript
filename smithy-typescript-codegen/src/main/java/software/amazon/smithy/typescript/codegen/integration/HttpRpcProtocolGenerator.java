...
public abstract class HttpRpcProtocolGenerator implements ProtocolGenerator {
   ...
   * value that can then be serialized by the implementation of {@code serializeInputDocument}. The
   * {@link DocumentShapeSerVisitor} and {@link DocumentMemberSerVisitor} are provided to reduce the
   ...
   * a value that can then be deserialized by the implementation of {@code
   * deserializeOutputDocument}. The {@link DocumentShapeDeserVisitor} and {@link
   * DocumentMemberDeserVisitor} are provided to reduce the effort of this implementation.
   *
   * @param context The generation context.
  ...
  public void generateSharedComponents(GenerationContext context) {
    ...
    Symbol requestSymbol =
        requestType.getSymbol().toBuilder().putProperty("typeOnly", false).build();
    ...
    writer.openBlock(
        ...
        requestType,
        () -> {
          // Get the hostname, port, and scheme from client's resolved endpoint. Then construct the
          // request from
          ...
          writer.openBlock(
              ...
              () -> {
                ...
                writer.write(
                    "path: basePath.endsWith(\"/\") ? basePath.slice(0, -1) + path : basePath +"
                        + " path,");
   ...
   *
   * <p>By default, headers are configured to use {@code SHARED_HEADERS}. See {@link
   * #writeSharedRequestHeaders}
   *
   * <pre>{@code
  ...
  protected boolean writeRequestBody(GenerationContext context, OperationShape operation) {
    if (operation.getInput().isPresent()) {
      ...
      StructureShape inputShape =
          context.getModel().expectShape(operation.getInput().get()).asStructureShape().get();
      TypeScriptWriter writer = context.getWriter();
      ...
      if (EventStreamGenerator.hasEventStreamInput(context, operation)) {
        MemberShape eventStreamMember =
            EventStreamGenerator.getEventStreamMember(context, inputShape);
   ...
   *
   * <p>Implementations of this method are expected to set a value to the {@code body} variable that
   * will be serialized as the request body. This variable will NOT be defined in scope and should
   * be defined by implementations if they wish to set it.
   *
   * <p>For example:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>Implementations should return true if they define a body variable, and false otherwise.
   *
   * @param context The generation context.
   * @param operation The operation being generated.
  ...
  protected void generateOperationDeserializer(
    ...
    writer.openBlock(
        ...
        () -> {
          ...
          if (SerdeElisionIndex.of(context.getModel()).mayElide(error) && enableSerdeElision()) {
            ...
            writer.write(
                "const deserialized: any = _json($L);", getErrorBodyLocation(context, "body"));
          } else {
            writer.write(
    ...
    OptionalUtils.ifPresentOrElse(
        ...
        outputId -> {
          ...
          StructureShape outputShape =
              context.getModel().expectShape(outputId).asStructureShape().get();
          if (EventStreamGenerator.hasEventStreamOutput(context, operation)) {
            MemberShape eventStreamMember =
                EventStreamGenerator.getEventStreamMember(context, outputShape);
            Shape target = context.getModel().expectShape(eventStreamMember.getTarget());
            Symbol targetSymbol = context.getSymbolProvider().toSymbol(target);
            writer.write(
