...
public final class ServiceBareBonesClientGenerator implements Runnable {
    ...
    writeInputOutputTypeUnion(
        ...
        writer -> {
          ...
          writer.addTypeImport(
              "MetadataBearer", "__MetadataBearer", TypeScriptDependency.SMITHY_TYPES);
  ...
  private void writeInputOutputTypeUnion(
    ...
    writer.indent();
    // If we have less symbols than operations, at least one doesn't have a type, so add the
    // default.
  ...
  private void generateConfig() {
    ...
    writer.addTypeImport(
        "SmithyConfiguration", "__SmithyConfiguration", TypeScriptDependency.AWS_SMITHY_CLIENT);
    writer.addTypeImport(
    ...
    List<SymbolReference> inputTypes =
        runtimePlugins.stream()
    ...
    if (!inputTypes.isEmpty()) {
      for (SymbolReference symbolReference : inputTypes) {
        if (symbolReference.getAlias().equals("EndpointInputConfig")) {
          writer.addTypeImport(
              "EndpointParameters", null, EndpointsV2Generator.ENDPOINT_PARAMETERS_DEPENDENCY);
  ...
  private void generateClientDefaults() {
    ...
    Runnable innerContent =
        () -> {
          writer.addTypeImport(
              "HttpHandlerUserInput", "__HttpHandlerUserInput", TypeScriptDependency.PROTOCOL_HTTP);
          writer.writeDocs(
              "The HTTP handler to use or its constructor options. Fetch in browser and Https in"
                  + " Nodejs.");
          writer.write("requestHandler?: __HttpHandlerUserInput;\n");

          writer.addTypeImport(
              "HashConstructor", "__HashConstructor", TypeScriptDependency.SMITHY_TYPES);
          writer.addTypeImport(
              "ChecksumConstructor", "__ChecksumConstructor", TypeScriptDependency.SMITHY_TYPES);
          writer.writeDocs(
              """
          ...
          writer.addTypeImport(
              "BodyLengthCalculator", "__BodyLengthCalculator", TypeScriptDependency.SMITHY_TYPES);
          writer.writeDocs(
              "A function that can calculate the length of a request body.\n" + "@internal");
          writer.write("bodyLengthChecker?: __BodyLengthCalculator;\n");

          writer.addTypeImport(
              "StreamCollector", "__StreamCollector", TypeScriptDependency.SMITHY_TYPES);
          writer.writeDocs(
  ...
  private void generateConstructor() {
    ...
    writer.openBlock(
        "constructor(...[configuration]: __CheckOptionalClientConfig<$L>) {",
        "}",
        configType,
        () -> {
          ...
          for (RuntimeClientPlugin plugin : runtimePlugins) {
            if (plugin.getResolveFunction().isPresent()) {
              ...
              Map<String, Object> paramsMap =
                  plugin.getAdditionalResolveFunctionParameters(model, service, null);
              ...
              writer.write(
                  "const $newConfig:L = $resolveFn:T($oldConfig:L" + additionalParamsString + ");");
              writer.popState();
            }
          }
          ...
          if (SchemaGenerationAllowlist.allows(service.getId(), settings)) {
            writer.addImportSubmodule(
                "getSchemaSerdePlugin", null, TypeScriptDependency.SMITHY_CORE, "/schema");
            writer.write(
                """
            ...
            plugin
                ...
                .ifPresent(
                    pluginSymbol -> {
                      // Construct additional parameters string
                      Map<String, Object> paramsMap =
                          plugin.getAdditionalPluginFunctionParameters(model, service, null);
    ...
    if (applicationProtocol.isHttpProtocol()) {
      writer.writeDocs(
          "Destroy underlying resources, like sockets. It's usually not necessary to do this.\n"
              + "However in Node.js, it's best to explicitly shut down the client's agent when it"
              + " is no longer needed.\n"
              + "Otherwise, sockets might stay open for quite a long time before the server"
              + " terminates them.");
    }
    writer.openBlock(
        "destroy(): void {",
        ...
        () -> {
          writer.pushState(
              ClientDestroyCodeSection.builder().runtimeClientPlugins(runtimePlugins).build());
          for (RuntimeClientPlugin plugin : runtimePlugins) {
            plugin
                .getDestroyFunction()
                .ifPresent(
