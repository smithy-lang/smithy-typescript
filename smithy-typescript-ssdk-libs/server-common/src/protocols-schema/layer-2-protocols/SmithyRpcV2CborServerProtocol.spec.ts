...
/**
 * Creates an operation schema with a simple struct output containing a string member.
 */
function makeStructOpSchema(): $OperationSchema {
  // Schema format: [structModifier | stringSchema, memberTraitsMap, memberName, innerSchema]
  // A simple struct with one string member "message".
  return {
    input: "unit",
    output: [
      // StructSchema: modifier=128(struct), members as entries
      [0, {}, "message", 0], // member "message" targeting string (schema=0)
    ],
    traits: {},
    namespace: "test",
    name: "MyOp",
...
describe("SmithyRpcV2CborServerProtocol", () => {
  ...
  describe("request validation - Smithy-Protocol header", () => {
    it("rejects requests without Smithy-Protocol header", async () => {
      ...
      await expect(
        protocol.deserializeRequest(makeUnitOpSchema(), makeContext(), request)
