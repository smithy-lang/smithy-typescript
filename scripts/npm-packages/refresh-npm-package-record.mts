 ...
 *
 * Runs on push to main so release PRs can skip the registry check for names and
 * versions already known to exist (see ensure-packages-exist-on-npm.mts). The
 * version recorded for a package is the one on main, which is the version the
 * next release PR supersedes. The record is carried between runs by the GitHub
 * Actions cache; only pushes to main may write to the cache scope that pull
 * requests restore from, which is why the refresh happens here and not on the
 * release PR itself.
 *
 * Versions that are not on npm are reported but never fail the run: the release
 * publishing them may still be in flight, and a package can legitimately sit on
 * main unpublished until a maintainer does its one-time manual first publish.
 * Enforcing that is the release PR check's job.
 *
