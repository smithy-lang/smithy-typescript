 ...
 * 1. Module-level: cycles among @smithy/* package imports across all packages.
 * 2. File-level: cycles among relative imports within each dist-cjs and dist-es folder.
 *
 * Usage: node cycles.js <packageDir> [...]
 */
 ...
 * node is reachable from every other. Any SCC with more than one node contains
 * at least one cycle. This function finds all SCCs, then traces one
 * representative cycle path through each.
 *
...
function findAllCycles(graph) {
  ...
  // Recursively assigns discovery indices and computes lowlinks
  ...
  // For each SCC, trace one cycle by DFS from an arbitrary start node
  // back to itself, staying within the component.
  const cycles = [];
  for (const component of connectedComponents) {
    ...
    // Walks edges within the component looking for a path back to start.
...
/**
 * Builds a module-level dependency graph from @smithy/* imports across packages.
 *
 * @param packageDirs - list of package root paths.
 * @returns adjacency list of package name -> Set of @smithy/* dependency names.
 */
async function buildModuleGraph(packageDirs) {
  ...
  for (const packageDir of packageDirs) {
    ...
    if (!name || !name.startsWith("@smithy/")) {
      continue;
    }
    if (!graph.has(name)) {
    ...
    for (const dist of ["dist-cjs", "dist-es"]) {
      ...
      for await (const file of walk(distDir, ["node_modules"])) {
        ...
        for (const specifier of extractImports(code)) {
          ...
          if (pkg.startsWith("@smithy/") && pkg !== name) {
...
/**
 * Resolves a self-referencing package import (e.g. "@smithy/core/client")
 ...
 * Follows both relative imports and self-referencing package imports.
 *
