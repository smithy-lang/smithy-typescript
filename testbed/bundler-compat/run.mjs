 ...
 *
 * These tests verify that @smithy packages can be successfully processed
...
const { findGlobalBufferRefs } = require("../bundlers/runner/bundler-output-analysis.cjs");
...
run("metro resolution", () => {
  ...
  const resolved = req.resolve("@smithy/smithy-rpcv2-cbor-schema");
  if (!resolved) throw new Error("Failed to resolve @smithy/smithy-rpcv2-cbor-schema");
});

run("metro bundle", () => {
  fs.mkdirSync(path.join(root, "dist/metro"), { recursive: true });
  execFileSync("node", [path.join(root, "metro-build.cjs")], { cwd: root, stdio: "pipe", timeout: 30_000 });
